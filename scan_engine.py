...
def compute_setup(data, sym, name):
    """NumPy-vectorized technical analysis and setup scoring in microseconds"""
    try:
        ...
        s20_arr = np.convolve(closes, np.ones(20)/20, mode='valid')
        s50_arr = np.convolve(closes, np.ones(50)/50, mode='valid')
        s200_arr = np.convolve(closes, np.ones(200)/200, mode='valid')

        s20_curr = float(s20_arr[-1])
        s20_prev = float(s20_arr[-2])
        s50_curr = float(s50_arr[-1])
        s50_prev = float(s50_arr[-2])
        s200_curr = float(s200_arr[-1])
        s200_prev = float(s200_arr[-2])

        dist_20 = round(((c_curr - s20_curr) / s20_curr) * 100, 1)
        dist_50 = round(((c_curr - s50_curr) / s50_curr) * 100, 1)
        dist_200 = round(((c_curr - s200_curr) / s200_curr) * 100, 1)
        chg_pct = round(((c_curr - c_prev) / c_prev) * 100, 2)
        ...
        elif s20_curr > s50_curr > s200_curr and c_curr > s20_curr and (lows_raw and lows_raw[-2] is not None and lows_raw[-2] <= s50_curr * 1.015):
        ...
        # Setup 4: Power Trend Bull Alignment
            ...
            score = 82 if (2.0 <= dist_20 <= 12.0) else 78
        ...
        tier = "A+" if score >= 90 else ("A" if score >= 75 else "B")
        ...
        return {
            ...
            "sma20": round(s20_curr, 2),
            "sma50": round(s50_curr, 2),
            "sma200": round(s200_curr, 2),
            "dist20Pct": dist_20,
            "dist50Pct": dist_50,
            "dist200Pct": dist_200,
            "volume": vol_curr,
...
async def run_async_scan(stocks, concurrency=45):
    ...
    print(f"Discovered {len(results)} high-probability setups.")

    payload = {
        "generatedAt": datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M:%S UTC"),
        "totalScanned": len(stocks),
        "totalSetupsFound": len(results),
        "executionSeconds": elapsed,
        "categories": {
            "all_top": results[:50],
            "golden_crosses": [r for r in results if r['setupType'] == 'GOLDEN_CROSS'][:30],
            "swing_breakouts": [r for r in results if r['setupType'] == 'SWING_BREAKOUT'][:30],
            "pullback_bounces": [r for r in results if r['setupType'] == 'PULLBACK_BOUNCE'][:30],
            "power_trends": [r for r in results if r['setupType'] == 'POWER_TREND'][:30]
        }
    }
    ...
    print(f"Top Setups Saved: {SCANNER_RESULTS_FILE}")
